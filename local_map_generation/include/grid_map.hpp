...
class GridMap {
 public:
    ...
    std::pair<int, int> getGridMapCoordinate(double px, double py) const {
        ...
        point.x_ = px;
        point.y_ = py;
        Point2f new_point = calcNewCoordinationPosition(curve_point, point);
    ...
    Point2f getCartesianCoordinate(int x, int y) const {
        ...
        Point2f raw_point = calcOldCoordinationPosition(curve_point, point);
        return raw_point;
    };
    ...
    cv::Mat toImage() const {
        cv::Mat image = convertVector2Mat<uchar>(this->data_, 1, this->height_);
        cv::Mat image_fliped;
